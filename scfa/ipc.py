...
class SCFAIPC:
    ...
    def clean(self) -> None:
        ...
        for path in [self.state_file, self.state_ready, self.cmd_file, self.cmd_ready, self.stop_file]:
            if path.exists():
                try:
                    path.unlink()
                except OSError:
    ...
    def wait_for_state(self, timeout_sec: float = 10.0) -> Optional[Dict[str, Any]]:
